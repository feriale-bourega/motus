...
var $motdico; // Déclaration de variable

 public function seemsUtf8($string) {  // Vérifie si une chaîne de caractères est codé en utf-8
      for ($i = 0; $i < strlen($string); $i++) { // La for boucle parcourt tous les octets et $str récupère ord le nombre décimal de l'octet courant.
        if (ord($string[$i]) < 0x80) continue; # 0bbbbbbb //vérifie si le 8e bit est ON présent dans l'octet courant.
    ...
    public static function unaccent($string){
        if ( ! preg_match('/[\x80-\xff]/', $string) ) { // Double échappement de caractères hexadécimaux
          return $string;
      }

      if (self::seemsUtf8($string)) {  // permet d'accéder aux constantes ou membres statiques d'une classe** depuis la même classe.
          $chars = array(
          // Decompositions for Latin-1 Supplement
          chr(195).chr(128) => 'A', chr(195).chr(129) => 'A',  //renvoie un **octet unique** contenant la valeur numérique de `$ascii`
 ...
 $chars['out'] = "EfSZszYcYuAAAAAACEEEEIIIINOOOOOOUUUUYaaaaaaceeeeiiiinoooooouuuuyy";
          ...
          $string = strtr($string, $chars['in'], $chars['out']); // Remplace des caractères dans une chaîne
          ...
          $string = str_replace($doubleChars['in'], $doubleChars['out'], $string); // Remplace toutes les occurences dans une chaîne
        }
